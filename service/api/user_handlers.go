...
func (rt *_router) startConversation(w http.ResponseWriter, r *http.Request, ps httprouter.Params, ctx reqcontext.RequestContext) {
	// 1. Get target user ID from URL path parameters
	targetUserID := ps.ByName("userId")

	// 2. Validate target user ID format
	if err := validateID(targetUserID, "userId"); err != nil {
	...
	_, err := rt.db.GetUserByID(targetUserID)
	if err != nil {
	...
	conversation, err := rt.db.GetOrCreateDirectConversation(currentUserID, targetUserID)
	if err != nil {
